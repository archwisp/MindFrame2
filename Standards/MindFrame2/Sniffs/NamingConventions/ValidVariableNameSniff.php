 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class MindFrame2_Sniffs_NamingConventions_ValidVariableNameSniff extends PHP_CodeSniffer_Standards_AbstractVariableSniff
{
   /**
   ...
   protected function processVariable(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
   {
      ...
      if ($tokens[$objOperator]['code'] === T_OBJECT_OPERATOR)
      {
         ...
         if ($tokens[$var]['code'] === T_STRING)
         {
            ...
            if ($tokens[$bracket]['code'] !== T_OPEN_PARENTHESIS)
            {
               ...
               if ($this->_isUnderScoreName($objVarName) === FALSE)
               {
                  $error = "Variable \"$originalVarName\" should not be in camelCase format";
      ...
      if ($this->_isUnderScoreName($varName) === FALSE)
      {
         $error = "Variable \"$originalVarName\" should not be in camelCase format";
   ...
   protected function processMemberVar(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
   {
      ...
      if ($this->_isUnderScoreName($varName) === FALSE)
      {
         $error = "Variable \"$varName\" should not be in camelCase format";
         $phpcsFile->addError($error, $stackPtr);
      }
      else if (preg_match('|\d|', $varName))
      {
   ...
   protected function processVariableInString(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
   {
      ...
      if (preg_match_all('|[^\\\]\$([a-zA-Z0-9_]+)|', $tokens[$stackPtr]['content'], $matches) !== 0)
      {
         foreach ($matches[1] as $varName)
         {
            ...
            {
               $varName = $matches[0];
               $error = "Variable \"$originalVarName\" " .
                  "should not be in camelCase format";

               $phpcsFile->addError($error, $stackPtr);
            }
            else if (preg_match('|\d|', $varName))
            {
               $warning = "Variable \"$originalVarName\" " .
                  "contains numbers but this is discouraged";
    ...
    * @return boolean
    */
   private function _isUnderscoreName($string)
   {
      ...
      else
      {
         foreach ($nameBits as $bit)
         {
            ...
            $first_char = substr($bit, 0, 1);

            if ($first_char !== strtolower($first_char))
            {
               $validName = FALSE;
               break;
            }
